...
# ===== CONFIG =====
# Owner IDs - Environment variable မရှိရင် default value သုံး
OWNER_IDS_STR = os.environ.get("OWNER_IDS", "8266394986")
...
# Owner Usernames
OWNER_USERNAMES = os.environ.get("OWNER_USERNAMES", "HEX_KING9,HANTHAR999").split(",")
OWNER_USERNAMES = [u.strip().lower() for u in OWNER_USERNAMES if u.strip()]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    button1 = InlineKeyboardButton(
        text="➕သင့်အဖွဲ့ထဲ ထည့်ရန်",
        url="http://t.me/myanmar_sayargyi_bot?startgroup=true"
    )

    button2 = InlineKeyboardButton(
        text="👑 𝘉𝘰𝘵 ĐÊV",
        url="tg://resolve?domain=HEX_KING9&text=Hello︎sir"
    )

    button3 = InlineKeyboardButton(
        text="𝑷𝒆𝒓𝒎𝒊𝒔𝒔𝒊𝒐𝒏 တောင်းရန်",
        url="https://t.me/myanmarbot_music/7"
    )
    ...
    button5 = InlineKeyboardButton(
        text="𝘉𝘰𝘵လိုချင်ရင်နှိိပ်",
        url="tg://resolve?domain=HEX_KING9&text=SIRရေးSpamBotအသစ်လုပ်ချင်လို့ပါ"
    )
...
app.add_handler(CommandHandler("attk", spam))
